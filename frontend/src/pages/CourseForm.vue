			...
			<div>
				...
				<div class="mt-5 mb-10">
					<div class="container mb-5">
						...
						<FormControl v-model="course.title" :label="__('Title')" class="mb-4" :required="true" />
						...
						<div class="mb-4">
							...
							<div class="flex items-center">
								...
								<FormControl v-model="newTag" :placeholder="__('Add a keyword and then press enter')"
									class="w-72" @keyup.enter="updateTags()" id="tags" />
							</div>
						</div>
	...
	makeParams(values) {
		return {
			doc: {
				...
				requested_departments: reqDepartments.value.map((reqDepartment) => ({
					requested_department: reqDepartment,
				})),
				...values,
			},
		}
...
const courseResource = createResource({
	...
	onSuccess(data) {
		Object.keys(data).forEach((key) => {
			...
			} else if (key == 'requested_departments') {
				reqDepartments.value = []
